...
struct ns_worker_ctx {
    ...
    // [0]: 记录 4K
    // [1]: 记录 256K
    uint32_t io_size_submitted_counter[2];
...
nvme_init_ns_worker_ctx(struct ns_worker_ctx *ns_ctx)
{
    ...
    // 初始化 IO 大小发送计数器
    ns_ctx->io_size_submitted_counter[0] = 0;
    ns_ctx->io_size_submitted_counter[1] = 0;
...
// 添加设置 epoll、rtc 模型 x
// 添加设置按大小分流时的混合比例 y
#define PERF_GETOPT_SHORT "a:b:c:d:e:ghi:lmo:q:r:k:s:t:w:z:A:C:DF:GHILM:NO:P:Q:RS:T:U:VZ:u:v:x:y:"
...
parse_args(int argc, char **argv, struct spdk_env_opts *env_opts)
{
    ...
    if (g_split_io_strategy != 0 && g_split_io_strategy != 1) {
        fprintf(stderr, "missing -u (--split-io-strategy) operand\n");
		usage(argv[0]);
		return 1;
    }
